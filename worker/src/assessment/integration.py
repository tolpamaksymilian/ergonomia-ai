...
SUPPORTED_METRICS_SCHEMA="1.0"; SUPPORTED_METRICS_VERSION="ergonomics-metrics-v1.0"; SUPPORTED_POSE_SCHEMAS=frozenset({"4.0", "5.0", "5.1", "6.0"})
...
def _validate_inputs(ergonomics:Mapping[str,Any],pose:Mapping[str,Any]|None)->str:
    ...
    if pose is not None:
        if pose.get("schema_version") not in SUPPORTED_POSE_SCHEMAS: raise AssessmentInputError("unsupported pose schema; assessment requires Pose V4, V5 or V6")
