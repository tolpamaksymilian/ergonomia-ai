...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await supabase.from("photo_scenes").update({
    ...
    scene_builder_version: "photo-scene-builder-v0.4-beta.1",
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || data !== true) return NextResponse.json({ error: "Nie można uruchomić analizy zdjęcia w bieżącym stanie." }, { status: 409 });
