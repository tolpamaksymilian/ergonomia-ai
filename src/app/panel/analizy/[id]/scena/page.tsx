...
export default async function PhotoScenePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: analysis, error } = await supabase.from("analyses")
    .select("id,title,analysis_type,status,processing_stage,error_message,source_image_path,workstation:workstations(name),analysis_category_links(category:analysis_categories(name))")
  ...
  const { data: scene, error: sceneError } = await supabase.from("photo_scenes")
    .select("scene_state,detection_result,image_width,image_height,detection_error_message,detection_version,last_saved_at")
