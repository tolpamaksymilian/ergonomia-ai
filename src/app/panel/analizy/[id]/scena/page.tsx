...
export default async function PhotoScenePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: scene, error: sceneError } = await supabase.from("photo_scenes")
    .select("scene_state,detection_result,image_width,image_height,detection_error_message,detection_version,detection_completed_at,detection_attempts,scene_builder_version,last_saved_at")
  ...
  return <main className="ui-page min-h-screen px-3 py-4 sm:px-6">
    <div className="mx-auto max-w-[1800px] space-y-4">
      <header className="ui-surface flex flex-wrap items-center justify-between gap-3 p-4">
        ...
        <div className="flex items-center gap-2"><span className="hidden rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary sm:inline-flex"><ImageIcon className="mr-2 size-4" />Scene Builder v0.4 Beta</span><ThemeToggle /></div>
      </header>
      <PhotoSceneEditor
