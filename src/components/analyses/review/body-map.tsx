      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-[9rem_1fr] lg:grid-cols-1 xl:grid-cols-[9rem_1fr]">
        <svg viewBox="0 0 160 330" className="mx-auto h-72 max-w-full" role="img" aria-label="Schemat sylwetki z aktywnym obszarem">
          <g fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round">
            ...
            <path d="M80 58v20M51 84c17-10 41-10 58 0l-7 93c-14 8-30 8-44 0z" className={activeArea === "trunk" ? "text-cyan-300" : "text-slate-600"} strokeWidth="12" />
            <path d="M80 58v22" className={activeArea === "neck" ? "text-cyan-300" : "text-slate-600"} strokeWidth="12" />
            <path d="M52 90L25 146 18 214" className={activeArea === "left_arm" || activeArea === "left_elbow" || activeArea === "left_wrist" ? "text-cyan-300" : "text-slate-600"} strokeWidth="12" />
            <path d="M108 90l27 56 7 68" className={activeArea === "right_arm" || activeArea === "right_elbow" || activeArea === "right_wrist" ? "text-cyan-300" : "text-slate-600"} strokeWidth="12" />
          ...
          {AREAS.map((area) => {
            ...
            return <circle key={area.id} cx={cx} cy={cy} r="7" className={activeArea === area.id ? "fill-cyan-300 stroke-cyan-100" : "fill-slate-800 stroke-slate-500"} strokeWidth="3" />;
          })}
        </svg>
        <div className="grid content-start grid-cols-2 gap-2 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-1">
          {AREAS.map((area) => (
            <button key={area.id} type="button" aria-pressed={selected === area.metric} onClick={() => onSelect(area.metric)} className={`min-w-0 rounded-xl border px-3 py-2.5 text-left text-xs font-semibold transition focus-visible:outline-2 focus-visible:outline-cyan-200 ${selected === area.metric ? "border-cyan-300/40 bg-cyan-300/10 text-cyan-100" : "border-white/[0.08] bg-white/[0.025] text-slate-400 hover:text-white"}`}>{area.label}</button>
          ))}
        </div>
      </div>
    </section>
