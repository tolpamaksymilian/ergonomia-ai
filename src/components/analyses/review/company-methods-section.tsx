...
export function CompanyMethodsSection({ analysisId, value, onSeek }: { analysisId: string; value: CompanyMethodsView | null; onSeek: (time: number) => void }) {
  ...
  return <section className="review-panel" aria-labelledby="company-methods-title">
    ...
    <div className="mt-2 flex flex-wrap items-end justify-between gap-3"><div><h2 id="company-methods-title" className="text-2xl font-semibold">OWAS i ocena kontekstowa</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Metody pozostają oddzielne od Risk Engine, RULA i REBA. Wartości manualne wymagają pomiaru lub potwierdzenia użytkownika.</p></div><span className="rounded-full border border-border bg-surface-muted px-3 py-1.5 text-xs font-semibold text-muted-foreground">{value?.version ?? "company-methods-v1.2-beta.1"}</span></div>
    ...
    {dominantFrame && <button type="button" onClick={() => onSeek(dominantFrame.time)} className="mt-4 rounded-xl border border-orange-200 bg-brand-soft px-4 py-3 text-left text-sm text-accent-foreground transition hover:bg-orange-100 dark:border-orange-800 dark:hover:bg-orange-950/45">Dominująca rozpoznana pozycja OWAS: kod {dominantFrame.code}, kategoria {dominantFrame.category}. Przejdź do {dominantFrame.time.toFixed(1)} s.</button>}
...
function MethodCard({ title, icon: Icon, status: value, detail }: { title: string; icon: typeof Weight; status: string; detail: string }) { const ready = ["AUTOMATIC", "MANUAL"].includes(value); return <article className="rounded-xl border border-border bg-card p-4"><div className="flex items-start justify-between gap-2"><Icon className="size-5 text-primary" /><span className={`rounded-full px-2 py-1 text-[9px] font-semibold uppercase ${ready ? "bg-emerald-50 text-emerald-800 dark:bg-emerald-950/35 dark:text-emerald-300" : "bg-amber-50 text-amber-800 dark:bg-amber-950/35 dark:text-amber-300"}`}>{statusLabel(value)}</span></div><h3 className="mt-4 font-semibold">{title}</h3><p className="mt-2 text-xs text-muted-foreground">{detail}</p></article>; }
