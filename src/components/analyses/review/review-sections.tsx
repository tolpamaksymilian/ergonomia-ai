          ...
          <div key={group}>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {metrics.filter((metric) => metric.group === group).map((metric) => (
                <button key={metric.name} type="button" onClick={() => onSelect(metric.name)} className="min-w-0 rounded-2xl border border-white/[0.08] bg-slate-950/35 p-4 text-left transition hover:border-cyan-300/30 hover:bg-cyan-300/[0.05] focus-visible:outline-2 focus-visible:outline-cyan-200">
...
export function KeyMomentsSection({ model, onSeek }: { model: AnalysisReviewModel; onSeek: (time: number) => void }) {
  ...
  return (
    <section className="review-panel" aria-labelledby="moments-title">
      ...
      {moments.length ? <div className="mt-6 grid gap-3 md:grid-cols-2">{moments.map((moment) => <article key={moment.id} className="flex min-w-0 gap-4 rounded-2xl border border-white/[0.08] bg-slate-950/35 p-4"><time className="shrink-0 font-mono text-sm font-semibold text-cyan-300">{formatTimestamp(moment.time)}</time><div className="min-w-0"><h3 className="font-semibold text-slate-100">{moment.title}</h3><p className="mt-1 text-sm leading-6 text-slate-400">{moment.description}</p><button type="button" onClick={() => onSeek(moment.time)} className="mt-3 text-xs font-semibold text-cyan-300 underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-cyan-200">Przejdź do filmu</button></div></article>)}</div> : <EmptyState text="Brak wiarygodnych momentów dla wybranego filtra." />}
    </section>
  );
}
