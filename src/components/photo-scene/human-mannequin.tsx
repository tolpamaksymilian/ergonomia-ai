...
export function HumanMannequin({ human, width, height, calibration, selected, reachVisible, reachMode, debug, zoom, onStart, onSelect }: {
  ...
  const pxPerCm = human.placement.lastScalePxPerCm ?? Math.max(0.4, height / 260);
  ...
  const limb = (side: "left" | "right") => <g key={side} opacity={leftBehind === (side === "left") ? 0.82 : 1}>
    <Capsule a={j[`${side}Hip`]} b={j[`${side}Knee`]} width={width} height={height} proximal={d.thighLengthCm * pxPerCm * 0.25} distal={d.thighLengthCm * pxPerCm * 0.18} fill="#263142" stroke={selectedStroke} />
    <Joint point={j[`${side}Knee`]} width={width} height={height} radius={d.lowerLegLengthCm * pxPerCm * 0.075} fill="#303b4c" />
    <Capsule a={j[`${side}Knee`]} b={j[`${side}Ankle`]} width={width} height={height} proximal={d.lowerLegLengthCm * pxPerCm * 0.18} distal={d.lowerLegLengthCm * pxPerCm * 0.12} fill="#202a39" stroke={selectedStroke} warning={human.pose.reachState[`${side}Leg`] === "OUT_OF_REACH"} />
    ...
    <Capsule a={j[`${side}Shoulder`]} b={j[`${side}Elbow`]} width={width} height={height} proximal={d.upperArmLengthCm * pxPerCm * 0.23} distal={d.upperArmLengthCm * pxPerCm * 0.16} fill="#2c3748" stroke={selectedStroke} />
    <Joint point={j[`${side}Elbow`]} width={width} height={height} radius={d.forearmLengthCm * pxPerCm * 0.07} fill="#344153" />
    <Capsule a={j[`${side}Elbow`]} b={j[`${side}Wrist`]} width={width} height={height} proximal={d.forearmLengthCm * pxPerCm * 0.18} distal={d.forearmLengthCm * pxPerCm * 0.11} fill="#222d3d" stroke={selectedStroke} warning={human.pose.reachState[`${side}Arm`] === "OUT_OF_REACH"} />
    <Hand wrist={j[`${side}Wrist`]} end={j[`${side}Hand`]} width={width} height={height} length={d.handLengthCm * pxPerCm} color={human.color} />
  </g>;

  if (human.placement.projectionStatus === "PROJECTION_INVALID") return <g data-human-id={human.id} onPointerDown={(event) => { event.stopPropagation(); onSelect(); }}><circle cx={human.placement.contactPoint.x * width} cy={human.placement.contactPoint.y * height} r="13" fill="rgba(15,23,42,.88)" stroke="#f59e0b" strokeWidth="3" strokeDasharray="5 4" /><path d={`M${human.placement.contactPoint.x * width - 5} ${human.placement.contactPoint.y * height - 5}l10 10m0-10l-10 10`} stroke="#fbbf24" strokeWidth="2" /></g>;
  ...
  return <g data-human-id={human.id} opacity={human.locked ? 0.72 : 1} onPointerDown={(event) => { event.stopPropagation(); onSelect(); }}>
    ...
    {debug && <Debug human={human} width={width} height={height} pxPerCm={pxPerCm} scaleStatus={human.placement.scaleStatus} ground={getGroundPlaneStatus(calibration)} />}
  </g>;
}
...
function Debug({ human, width, height, pxPerCm, scaleStatus, ground }: { human: SceneHuman; width: number; height: number; pxPerCm: number; scaleStatus: string; ground: string }) { const p = human.placement.contactPoint; return <g pointerEvents="none"><rect x={p.x * width + 16} y={p.y * height - 112} width="238" height="101" rx="7" fill="rgba(2,6,23,.88)" stroke="#fb923c" /><text x={p.x * width + 24} y={p.y * height - 94} fill="#fed7aa" fontSize="10"><tspan x={p.x * width + 24}>Digital Human v1 · Scene World Model</tspan><tspan x={p.x * width + 24} dy="14">height: {human.profile.heightCm.toFixed(1)} cm / {renderedHeightPixels(human.pose, height).toFixed(1)} px</tspan><tspan x={p.x * width + 24} dy="14">vertical scale: {pxPerCm.toFixed(3)} px/cm · {scaleStatus}</tspan><tspan x={p.x * width + 24} dy="14">source: {human.placement.scaleReferences.join(", ") || "display fallback"}</tspan><tspan x={p.x * width + 24} dy="14">coverage: {human.placement.calibrationCoverage} · back: {human.placement.backConvertedHeightCm?.toFixed(1) ?? "—"} cm</tspan><tspan x={p.x * width + 24} dy="14">foot: {p.x.toFixed(3)}, {p.y.toFixed(3)} · yaw {human.placement.orientationDeg.toFixed(0)}°</tspan><tspan x={p.x * width + 24} dy="14">{ground} · IK {human.pose.reachState.leftArm}/{human.pose.reachState.rightArm}</tspan></text></g>; }
