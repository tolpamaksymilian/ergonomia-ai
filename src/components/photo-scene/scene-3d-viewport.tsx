...
const BODY_SEGMENTS: [Human3DJointName,Human3DJointName,string][] = [
  ["pelvis","spineLower","torso"],["spineLower","spineMid","torso"],["spineMid","chest","torso"],["chest","neck","neck"],["neck","head","head"],["head","headTop","head"],
  ["chest","leftClavicle","arm"],["leftClavicle","leftShoulder","arm"],["leftShoulder","leftElbow","arm"],["leftElbow","leftWrist","forearm"],["rightClavicle","rightShoulder","arm"],["chest","rightClavicle","arm"],["rightShoulder","rightElbow","arm"],["rightElbow","rightWrist","forearm"],
  ["pelvis","leftHip","pelvis"],["leftHip","leftKnee","thigh"],["leftKnee","leftAnkle","calf"],["leftAnkle","leftFoot","foot"],["pelvis","rightHip","pelvis"],["rightHip","rightKnee","thigh"],["rightKnee","rightAnkle","calf"],["rightAnkle","rightFoot","foot"],
];

export function Scene3DViewport({ humans, objects, selectedHumanId, debug = false }: { humans: SceneHuman[]; objects: SceneObject[]; selectedHumanId: string|null; debug?: boolean }) {
  ...
  return <div className="relative min-h-[520px] overflow-hidden rounded-xl bg-slate-950" role="application" aria-label="Interaktywny widok stanowiska 3D w centymetrach">
    ...
    <Canvas shadows camera={{ position: cameraPosition(preset), fov: 38, near: 1, far: 2500 }} key={preset} dpr={[1,1.5]}>
      ...
      {humans.filter((h)=>h.visible).map((human)=><HumanMesh key={human.id} human={human} selected={human.id===selectedHumanId} debug={debug}/>) }
      <OrbitControls makeDefault target={[0,90,0]} enableDamping minDistance={90} maxDistance={900} maxPolarAngle={Math.PI*.49}/>
    </Canvas>
  </div>;
}

function HumanMesh({human,selected,debug}:{human:SceneHuman;selected:boolean;debug:boolean}){
  ...
  return <group>{BODY_SEGMENTS.map(([a,b,kind])=><Segment key={`${a}-${b}`} start={joints[a]} end={joints[b]} radius={radius(kind)} color={selected?human.color:"#64748b"}/>) }
    ...
    {fingers.map((finger)=><Segment key={`${finger.side}-${finger.name}-${finger.index}`} start={finger.start} end={finger.end} radius={Math.max(.6,d.handWidthCm*.07)*(1-finger.index*.12)} color={selected?"#67e8f9":"#94a3b8"}/>) }
