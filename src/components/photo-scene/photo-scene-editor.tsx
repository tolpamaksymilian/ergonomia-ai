...
function HumanEditor({ human, state, update, imageWidth, imageHeight }: { human: SceneHuman; state: SceneState; update: (fn: (state: SceneState) => SceneState) => void; imageWidth: number; imageHeight: number }) {
  ...
  function rebuild(profile = human.profile, posture = human.pose.preset, orientation = human.placement.orientationDeg, facingPreset = human.placement.facingPreset) { const candidate = { ...human, visible: true, profile, constraints: createConstraintGraph(profile), pose: { ...human.pose, preset: posture }, placement: { ...human.placement, orientationDeg: orientation, facingPreset } }; const projected = placeSingleHuman(candidate, human.placement.contactPoint, state.calibration, imageWidth, imageHeight, orientation, human.placement.positionMode, human.placement.attachedObjectId); replace({ ...projected, placement: { ...projected.placement, facingPreset } }); }
